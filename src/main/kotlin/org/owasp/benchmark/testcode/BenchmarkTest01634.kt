...
class BenchmarkTest01634 : HttpServlet() {
    ...
    public override fun doPost(request: HttpServletRequest, response: HttpServletResponse) {
        ...
        if (paramLoc == -1) {
            response.writer.println("getQueryString() couldn't find expected parameter '" + "BenchmarkTest01634" + "' in query string.")
            return
        }
        var param: String? =
            queryString!!.substring(paramLoc + paramval.length) // 1st assume "BenchmarkTest01634" param is last parameter in query string.
...
//	    };
    ...
    private inner class Test {
        ...
        fun doSomething(request: HttpServletRequest?, param: String): String {
            ...
            val e37769 = String(
                Base64.decodeBase64(
                    Base64.encodeBase64(d37769.toByteArray())
                )
            ) // B64 encode and decode it
            val f37769 = e37769.split(" ".toRegex()).toTypedArray()[0] // split it on a space
            val thing = ThingFactory.createThing()
            val g37769 = "barbarians_at_the_gate" // This is static so this whole flow is 'safe'
            return thing.doSomething(g37769) // reflection
        }
    } // end innerclass Test
