...
class BenchmarkTest01638 : HttpServlet() {
    ...
    public override fun doPost(request: HttpServletRequest, response: HttpServletResponse) {
        ...
        if (paramLoc == -1) {
            response.writer.println("getQueryString() couldn't find expected parameter '" + "BenchmarkTest01638" + "' in query string.")
            return
        }
        var param: String? =
            queryString!!.substring(paramLoc + paramval.length) // 1st assume "BenchmarkTest01638" param is last parameter in query string.
...
//	    };
    ...
    private inner class Test {
        ...
        fun doSomething(request: HttpServletRequest?, param: String): String {
            ...
            val e61263 = String(
                Base64.decodeBase64(
                    Base64.encodeBase64(d61263.toByteArray())
                )
            ) // B64 encode and decode it
            val f61263 = e61263.split(" ".toRegex()).toTypedArray()[0] // split it on a space
            val thing = ThingFactory.createThing()
            return thing.doSomething(f61263) // reflection
        }
    } // end innerclass Test
