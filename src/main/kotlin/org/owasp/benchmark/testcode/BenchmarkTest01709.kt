...
class BenchmarkTest01709 : HttpServlet() {
    ...
    public override fun doPost(request: HttpServletRequest, response: HttpServletResponse) {
        ...
        if (paramLoc == -1) {
            response.writer.println("getQueryString() couldn't find expected parameter '" + "BenchmarkTest01709" + "' in query string.")
            return
        }
        var param: String? =
            queryString!!.substring(paramLoc + paramval.length) // 1st assume "BenchmarkTest01709" param is last parameter in query string.
    ...
    private inner class Test {
        ...
        fun doSomething(request: HttpServletRequest?, param: String): String {
            ...
            val e15574 = String(
                Base64.decodeBase64(
                    Base64.encodeBase64(d15574.toByteArray())
                )
            ) // B64 encode and decode it
            val f15574 = e15574.split(" ".toRegex()).toTypedArray()[0] // split it on a space
            val thing = ThingFactory.createThing()
            return thing.doSomething(f15574) // reflection
        }
    } // end innerclass Test
