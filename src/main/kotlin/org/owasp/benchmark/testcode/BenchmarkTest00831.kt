...
class BenchmarkTest00831 : HttpServlet() {
    ...
    public override fun doPost(request: HttpServletRequest, response: HttpServletResponse) {
        ...
        if (paramLoc == -1) {
            response.writer.println("getQueryString() couldn't find expected parameter '" + "BenchmarkTest00831" + "' in query string.")
            return
        }
        var param =
            queryString!!.substring(paramLoc + paramval.length) // 1st assume "BenchmarkTest00831" param is last parameter in query string.
        ...
        val e97193 = String(
            Base64.decodeBase64(
                Base64.encodeBase64(d97193.toByteArray())
            )
        ) // B64 encode and decode it
        val f97193 = e97193.split(" ".toRegex()).toTypedArray()[0] // split it on a space
