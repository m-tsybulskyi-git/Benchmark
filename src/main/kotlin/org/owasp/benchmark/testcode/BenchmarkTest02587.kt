...
class BenchmarkTest02587 : HttpServlet() {
    ...
    public override fun doPost(request: HttpServletRequest, response: HttpServletResponse) {
        ...
        if (paramLoc == -1) {
            response.writer.println("getQueryString() couldn't find expected parameter '" + "BenchmarkTest02587" + "' in query string.")
            return
        }
        var param =
            queryString!!.substring(paramLoc + paramval.length) // 1st assume "BenchmarkTest02587" param is last parameter in query string.
    ...
    companion object {
        ...
        private fun doSomething(request: HttpServletRequest, param: String): String {
            ...
            val e93081 = String(
                Base64.decodeBase64(
                    Base64.encodeBase64(d93081.toByteArray())
                )
            ) // B64 encode and decode it
            val f93081 = e93081.split(" ".toRegex()).toTypedArray()[0] // split it on a space
            val thing = ThingFactory.createThing()
            return thing.doSomething(f93081) // reflection
        }
    }
}